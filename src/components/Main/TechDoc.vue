...
<template>
    <div class="page-container">
        <div class="content-container mt-2">
          ...
          <p>如同所有人工智慧技術，訓練資料(Training Data)的特色與品質往往是決定其應用成效的首要因素。相較於其他類型的文本資料，法院裁判作為自然語言處理的應用有幾個好處：首先，裁判書類係由法學專業的法官所撰寫，以嚴謹的邏輯與法律用語處理社會之爭端。因此其用字遣詞不但有其條理性與穩定性，內容又可呈現出當代社會生活的情境。更重要者，我國為成文法國家，民法及相關法規相當詳盡，很適合做為機器學習的輔助；在司法實務上，多數民事案件也能加以類型化。因此，如果人工智慧能在民事裁判的判讀、分類與預測上有所進展，咸信有助於減緩一般民眾對司法裁判期待的落差，達到止謗息爭的果效，間接提升人民對司法之信賴。</p>
          <h6>二、計畫簡介</h6>
          <p>目前我們的研究先集中於研究親權裁定(即當父母雙方離婚後如何酌定子女的監護權)的案件。例如究竟民法1055-1條中所強調的「子女最佳利益」在司法實務上是如何被實踐評估，能否由AI來學習判斷？研究的三個大方向是(1)建構判決書的標註資料庫，(2)研發自然語言處理的模型，以及(3)發展法學實證相關研究。本計畫是屬於科技部AI人文社會領域計畫，由清華大學所執行的「競爭或合作？人工智慧在人文社會的應用與影響探究」總計畫中的子計畫一「可解釋性人工智慧在家事裁判之應用與其限制」。計畫執行期間為2019-2020。</p>
          <p>本研究的判決書資料來自於司法院法學資料檢索系統。第一階段是蒐集2015-2017這三年間各級法院所有與親權相關的案件。此處的「親權相關」乃指全文帶有「離婚」、「審酌」與「子女最佳利益」的判決，共計2,620篇。未來會再擴展至其他年份與類型。我們先以人工的方式將這些判決書標註出「類型特徵」與「理據特徵」。前者包括判決結果、雙方意願、雙方身分、雙方國籍；後者包括對於雙方有利與不利的文字理由段落。有機會成為國內目前最詳盡的民事案件分析資料庫。這些資料形成原來判決書的「等效資料」，可以更方便地用來作進一步的量化與質性研究。其中也包括利用自然語言處理的方式來作判決預測。</p>
          <div class="d-block">
            <img class="w-25 mx-auto d-block" src="../../../static/project_intro.png" />
          </div>

          <h6>三、模型輸入資料的介紹</h6>
          <p>本研究所開發的模型，皆是基於下列兩種標注資料作為模型輸入：其一為申請方與相對方各自有利/不利的理據，以及從雙方有利/不利的理據中，所標註出的十三個親權判決因子('親子感情', '意願能力', '父母經濟', '支持系統', '父母生活', '主要照顧','子女年齡', '人格發展', '父母健康', '父母職業', '子女意願', '友善父母', '父母品行')。</p>
          <h6>四、模型介紹與評估</h6>
          <p>在完整的資料收集和標註之後，我們開始著手於自然語言處理模型的研發。為了能夠充分探索並評估不同模型對於判決書的預測效果，我們在本網頁展示四種基於不同深度學習算法所開發的親權判決模型，分別是：模式一的 L1，模式二的 S1, 模式三的 C1，而在模式一、二、三所展示的 L2, S2, C2 則都對應到同一個模型。以下針對這五種模型的架構、算法以及效能進行詳細介紹：</p>
          <ul>
            <li>(模式一)Model L1:
              <p>L1 為基於 XGBOOST 所開發的模型。XGBOOST 是一種優化的梯度增強機學習算法，它在多個機器學習競賽和實際應用中都有出色的表現。我們利用其對於非線性關係的建模能力，將判決書中的親權判決因子進行one-hot編碼與相關特徵的轉換後，作為模型的輸入並試圖預測判決結果。</p>
            ...
            <li>
              (模式三)Model C1:
              <p>我們也探索了使用 Lawformer 作為基礎模型來進行 fine-tuning。Lawformer 是一個特別為法律文本設計和訓練的語言模型。相較於一般的語言模型，Lawformer 更能夠理解和捕捉法律文本中的特定語境和專業詞彙，從而提供更高的預測準確性和解釋性。我們首先從 Lawformer 預訓練的模型開始，然後對其進行微調，使其能夠適應我們的判決書資料集。這個模型的優勢在於，由於它已經是在法律相關的文本上進行訓練的，所以在進行微調時，它可以更快地適應和學習我們資料集中的特定特徵和模式。</p>
            </li>
            <li>
